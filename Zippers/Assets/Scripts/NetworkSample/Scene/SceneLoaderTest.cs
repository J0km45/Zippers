...
public class SceneLoaderTest : MonoBehaviour
{
    ...
    /// <summary>
    /// 로컬 씬 로드 (SceneIdTest 권장). NGO 미실행 상태에서 사용
    /// </summary>
    public static void LoadLocal(SceneIdTest id) => LoadLocal(id.GetName());
    ...
    /// <summary>
    /// NGO 동기화 씬 로드 (SceneIdTest 권장). 호스트에서만 호출하면 모든 멤버에게 자동 전파됨
    /// </summary>
    public static bool LoadNetworked(SceneIdTest id) => LoadNetworked(id.GetName());
    ...
    public static bool LoadNetworked(string sceneName)
    {
        ...
        {
            Debug.LogError($"SceneLoaderTest: NGO 동기화 로드 실패 - Host가 아니거나 SceneManager 없음 (scene='{sceneName}')");
            return false;
        }
